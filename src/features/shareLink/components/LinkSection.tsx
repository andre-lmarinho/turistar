...
export function LinkSection({ planId }: { planId: string }) {
  ...
  const buttonLabel = hasLink ? (copied ? "Copied" : "Copy link") : "Create link";
  const buttonClassName = `inline-flex cursor-pointer items-center gap-2 hover:underline disabled:cursor-not-allowed disabled:opacity-60 ${
