...
describe("fetchGeoapifyPlaceSearch", () => {
  beforeEach(async () => {
    vi.resetModules();
    process.env.NEXT_PUBLIC_GEOAPIFY_KEY = "test-key";
    ({ fetchGeoapifyPlaceSearch } = await import("@/features/app/planner/services/geoapify/placeSearch"));
  });
  ...
  it("maps search results to a simplified shape", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith(expect.stringContaining("text=Forte"), expect.any(Object));
    expect(results).toEqual([
      {
        placeId: "123",
        name: "Forte de Monte Serrat",
...
describe("fetchGeoapifyPlaceDetails", () => {
  beforeEach(async () => {
    vi.resetModules();
    process.env.NEXT_PUBLIC_GEOAPIFY_KEY = "test-key";
    ({ fetchGeoapifyPlaceDetails } = await import("@/features/app/planner/services/geoapify/placeDetails"));
  });
