...
describe("LocationSearchInput", () => {
  ...
  it("renders an error message when the hook returns an error", () => {
    ...
    render(<LocationSearchInput value="Paris" onChange={() => {}} autocompleteHook={autocompleteHook} />);

    expect(screen.getByText("Failed to load location suggestions.")).toBeInTheDocument();
  });
});
