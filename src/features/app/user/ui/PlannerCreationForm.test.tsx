...
const saveRecentPlan = vi.fn();
const createPlanFn = vi.fn();
const onPlanCreated = vi.fn();

const { triggerLocationChange, getCapturedProps, LocationSearchInput } = vi.hoisted(() => {
  ...
  return {
    ...
    LocationSearchInput: (props: {
      value: string;
      onChange: (value: string | AutocompletePlace) => void;
      placeholder: string;
      autocompleteHook?: unknown;
    }) => {
      capturedProps = props;
...
describe('PlannerCreationForm', () => {
  beforeEach(() => {
    ...
    saveRecentPlan.mockReset();
    createPlanFn.mockReset();
    onPlanCreated.mockReset();
  });

  describe('wiring', () => {
    it('passes destination autocomplete hook to LocationSearchInput', () => {
      render(<PlannerCreationForm persistEditTokens={false} createPlanFn={createPlanFn} />);
  ...
  describe('behavior', () => {
    ...
    it('shows a failure message when plan creation rejects', async () => {
      ...
      await waitFor(() => expect(screen.getByPlaceholderText('Destination')).toHaveValue('City'));
    ...
    it('calls onPlanCreated instead of pushing when provided', async () => {
      ...
      await act(async () => {
        triggerLocationChange({
          ...
          longitude: 0,
          country: 'IT',
        } as unknown as AutocompletePlace);
      });
