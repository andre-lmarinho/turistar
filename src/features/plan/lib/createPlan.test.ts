...
import { createPlan } from "@/features/plan/lib/createPlan";
...
describe("createPlan action", () => {
  ...
  it("throws when the RPC returns an error", async () => {
    ...
    await expect(createPlan("x", { name: "Y" }, "2024-01-01", "2024-01-02")).rejects.toThrow(
      'Failed to create plan: operation=createPlan title="x" destination="Y" userId=null error=failed'
    );
  });
  ...
  it("continues when Geoapify lookup fails and leaves country null", async () => {
    ...
    expect(rpc).toHaveBeenCalledWith(
      "create_full_plan",
      expect.objectContaining({ _dest_country: undefined })
    );
  });
