    ...
    <section
      ...
      <div className="flex flex-col gap-4">
        <h2 className="flex items-center gap-4">
          <span id={headingId} className="text-muted-foreground text-sm font-medium tracking-wide">
            {title}
          </span>
        ...
        <div
          className={["my-4 border-t border-dashed", emphasis ? "border-primary/20" : "border-border"].join(
            " "
          )}
        />
      </div>
      ...
      {/* Feature list */}
      <div>
        <p className="text-muted-foreground mb-3 text-xs font-medium tracking-wide">{sectionLabel}</p>
        <ul className="space-y-3">
          {features.map((f) => (
            <FeatureItem key={f.label}>{f.label}</FeatureItem>
          ))}
        </ul>
      </div>
    </section>
