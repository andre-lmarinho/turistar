...
- **Commit style**: Start commits with an appropriate Gitmoji followed by a conventional type and a short imperative description in English (e.g., `✨ feat: add map view`). A commitlint hook enforces this format, and commit suggestions after PR/merge must also use Gitmoji.
